...
2. **Checklist items are greppable when possible.** "No `pickle.loads` on untrusted data" beats "Avoid unsafe deserialization."
...
9. **No prose without a pattern.** If you cannot turn the advice into a Red Flag or Checklist item, it does not belong here.
10. **Test the module by reviewing a real PR with it.** If checklist items did not fire when they should, fix the wording.
