...
- When you disagree with a finding, push back with a `file:line` of the safe pattern. After the run,
  the skill asks whether to persist any of this run's outcomes (project-specific safe patterns, false
  positives, severity overrides, out-of-scope paths) to `.claude/security-review.md` in the target
  repo. Approve per entry; the file is human-edited, never overwritten.

## Project Notes (`.claude/security-review.md`)

Per-project learnings live in `.claude/security-review.md` at the **target repo's** root, not in this
skill's source tree. The skill reads it at scan start and uses it to skip known-safe patterns, suppress
known false positives, adjust severity for the codebase, and exclude scoped-out paths. Both
`py-security` and `node-security` share the file.
