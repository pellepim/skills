...
applies_to:
  ...
  # OR
  - framework: <e.g. express, fastify, nestjs, koa, next-api>
  # OR
  - dependency: <npm package name to detect via package.json / lockfile>
  # OR
...
2. **Checklist items are greppable when possible.** "No `child_process.exec` with template literals" beats "Avoid command
   injection."
